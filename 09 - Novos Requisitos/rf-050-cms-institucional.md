...
| RF-050.10 | O sistema deve exibir histórico de versões com as últimas 20 versões, exibindo: data, autor e botão "Restaurar"                             |
...
| RF-050.13 | O editor deve bloquear tags `<script>` e atributos de evento inline (proteção XSS)                                                          |
...
| RF-050.24 | O sistema deve permitir reordenar categorias via drag-and-drop ou campo de ordem numérico                                          |
...
| RF-050.32 | O sistema deve permitir reordenar perguntas dentro da categoria via drag-and-drop                                                      |
...
| RN-050.04 | O histórico de versões retém as últimas **20 versões** por seção (Política, Condições)                                               |
| RN-050.05 | Ao restaurar versão anterior, a versão corrente é automaticamente salva como rascunho antes da substituição                          |
| RN-050.06 | O editor WYSIWYG deve bloquear tags `<script>` e atributos de evento (ex.: `onclick`, `onload`) para prevenir XSS                   |
...
| RN-050.10 | Slug de categoria é **auto-gerado** a partir do nome (ex.: "Devolução" → "devolucao"), editável manualmente; deve ser único         |
...
| CA-050.02 | Editar Política de Privacidade e clicar em "Salvar rascunho"                      | Conteúdo salvo sem publicar; portal exibe versão anterior; indicador de rascunho não publicado  |
| CA-050.03 | Clicar em "Publicar" na Política de Privacidade                                   | Modal de confirmação exibido; ao confirmar: portal atualizado imediatamente                     |
...
| CA-050.06 | Tentar inserir `<script>` no editor WYSIWYG                                       | Tag bloqueada; conteúdo sanitizado ao salvar                                                    |
...
| CA-050.12 | Reordenar perguntas via drag-and-drop                                             | Nova ordem refletida no portal após publicação                                                  |
