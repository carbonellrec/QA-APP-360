...
| RF-052.03 | O sistema deve exibir painel de resumo com: total de notificações enviadas no mês, taxa média de abertura de e-mail, taxa de entrega push |
| RF-052.04 | O sistema deve exibir indicador de status do serviço de envio (Operacional / Degradado / Offline)                                    |
...
| RF-052.24 | A condição de opt-in deve ter duas opções: "Obrigatório" (envia independente de preferências) e "Respeita opt-in" (verifica RF-038)        |
...
| RF-052.30 | O formulário deve exibir seleção de segmentação do público-alvo: Todos os turistas / Por categoria de preferência / Por cidade / Por histórico de compra |
...
| ID        | Requisito                                                                                                                               |
|-----------|-----------------------------------------------------------------------------------------------------------------------------------------|
| RF-052.36 | O sistema deve exibir log de todas as notificações enviadas (transacionais e campanhas) com colunas: Data/Hora, Turista, Evento/Campanha, Canal, Status |
| RF-052.37 | Os status possíveis de cada envio são: Enviado, Aberto, Falhou, Bounced                                                                |
| RF-052.38 | O sistema deve exibir filtros: Canal, Status, Evento/Campanha, Período                                                                  |
| RF-052.39 | O sistema deve exibir taxa de abertura por campanha de e-mail e taxa de entrega por campanha push                                      |
| RF-052.40 | O sistema deve exibir botão "Exportar CSV" para o histórico conforme filtros aplicados                                                  |

Tipos de Evento

| Evento                       | Categoria        | Opt-out pelo Turista |
|------------------------------|------------------|----------------------|
| Compra Confirmada            | Transacional     | Não permitido        |
| Reembolso Aprovado           | Transacional     | Não permitido        |
| Reembolso Rejeitado          | Transacional     | Não permitido        |
| Senha Redefinida             | Transacional     | Não permitido        |
| Lembrete de Visita           | Opt-in           | Permitido (RF-038)   |
| Promoção / Oferta Especial   | Marketing/Opt-in | Permitido (RF-038)   |
| Nova Atração Disponível      | Marketing/Opt-in | Permitido (RF-038)   |
| Cupom de Desconto            | Marketing/Opt-in | Permitido (RF-038)   |
...
| RN-052.03 | Notificações de marketing respeitam obrigatoriamente as preferências de opt-in do turista (RF-038 / LGPD); turista que optou por não receber não receberá |
...
| RN-052.12 | Excluir template não exclui o histórico de envios vinculado; o template aparece como "[Excluído]" no histórico                                  |

Critérios de Aceitação

| ID        | Critério                                                                                      | Resultado Esperado                                                                                       |
|-----------|-----------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------|
| CA-052.01 | Acessar Central de Notificações como Administrador                                            | Sub-menu com 4 abas exibido; painel de resumo com métricas do mês visível                               |
