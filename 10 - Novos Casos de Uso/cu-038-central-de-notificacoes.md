...
Atores Secundários: Sistema (serviços de envio de e-mail e push, validação de variáveis, integração com preferências de opt-in RF-038)

Pré-condições: Usuário autenticado com perfil Administrador

Pós-condições: Templates criados/atualizados; regras de disparo automático configuradas; campanhas criadas (imediatas ou agendadas) ou canceladas; histórico de envios disponível para consulta e exportação
...
2. O sistema exibe sub-menu com 4 abas (Templates, Regras de Disparo, Campanhas, Histórico de Envios), painel de resumo do mês e indicador do status do serviço de envio.
...
4. Em **Regras de Disparo**, o ator associa por tipo de evento o canal (E-mail/Push/Ambos), o template, o delay e a condição de opt-in (Obrigatório ou Respeita opt-in); pode ativar/desativar regras individualmente.
5. Em **Campanhas**, o ator clica em "Nova Campanha", informa nome, tipo, template, segmentação do público-alvo (com estimativa de alcance) e data/hora de envio (Imediato ou Agendado); confirma a criação.
