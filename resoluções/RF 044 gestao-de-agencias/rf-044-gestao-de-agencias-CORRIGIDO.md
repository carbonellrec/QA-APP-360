...
| RN-044.14 | Ao aprovar uma agência, o sistema deve gerar automaticamente um rascunho de **Contrato de Agência** pré-preenchido no módulo de Gestão de Contratos (RF-008), com os dados cadastrados na RF-045 (razão social, CNPJ, responsável, dados bancários). O contrato é criado com status "Aguardando contrato", pronto para revisão e envio ao DocuSign pelo Administrador |
