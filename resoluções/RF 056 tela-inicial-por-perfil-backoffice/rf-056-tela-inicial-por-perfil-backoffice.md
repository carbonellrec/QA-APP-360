...
| RF-056.18 | O menu lateral do Editor deve conter os seguintes módulos: Totais da Atração, Validar Ingresso, Pesquisar Ingresso, Editar Atração, Gestão de Ingressos, Categorias, Gestão de Cupons, Gráficos e Analytics, Perfil |
...
| RF-056.25 | Todos os dados de cadastro visíveis ao Leitor devem ser somente leitura; o perfil não possui permissão para criar, editar ou excluir nenhum registro cadastral. A ação de Validar Ingresso (RF-025) é uma permissão funcional específica do perfil e não constitui edição de dados cadastrais (ver RN-056.15) |
...
| RF-056.31 | O perfil Agência não deve ter acesso aos módulos exclusivos do Parceiro Comercial (Totais da Atração, Pesquisar Ingresso, Gestão de Ingressos, Gestão de Cupons, Gráficos e Analytics, Editar Atração, Usuários da Atração, Relatórios da Atração, Negociação Financeira, Resumo de Despesas), do Editor/Leitor (Validar Ingresso, Categorias) nem ao painel de Administrador (Dashboard RF-003, Gestão Global de Usuários RF-006, Gestão de Contratos RF-008, Configurações Comerciais RF-010, Gestão Global de Atrações RF-012, conforme RN-056.08) |
...
| Editor | Totais da Atração (RF-004), Validar Ingresso (RF-025), Pesquisar Ingresso (RF-015/016), Editar Atração (RF-020), Gestão de Ingressos (RF-017), Categorias (RF-014), Gestão de Cupons (RF-018), Gráficos e Analytics (RF-019), Perfil (RF-005) | Totais da Atração (RF-004) |
...
| Administrador* | Não definido neste RF — ver RF-002/RF-003 | Dashboard do Administrador (RF-003) |

*A linha "Administrador" é apenas referencial: por força de RN-056.13, este RF não governa o perfil Administrador. A definição de como o Administrador visualiza módulos compartilhados com outros perfis (ex.: RF-047 — Operação Comercial, com visão de todas as agências/agentes; RF-048 — Comissionamento e Repasse, com visão de todos os repasses) deve constar em RF-002, RF-003, RF-047 ou RF-048, e não neste documento.
...
| RN-056.15 | (Exceção de Leitura — Validar Ingresso) Embora o perfil Leitor possua acesso somente leitura aos dados cadastrais da atração (RN-056.05), a ação de Validar Ingresso (RF-025) é uma permissão funcional específica do perfil e não constitui criação, edição ou exclusão de registro cadastral, não contrariando a restrição de somente leitura |
...
| CA-056.06 | Realizar login com credenciais de Leitor | Sistema redireciona para Validar Ingresso (RF-025); menu lateral exibe os 3 módulos do perfil; nenhum botão de criação, edição ou exclusão de registros cadastrais é exibido, exceto a ação de Validar Ingresso, que permanece disponível (RN-056.15) |
...
| CA-056.16 | Usuário recolhe o menu lateral, fecha o navegador e realiza login novamente em uma nova sessão | Menu lateral é renderizado no mesmo estado (recolhido) configurado na sessão anterior, conforme preferência persistida (RN-056.11) |
